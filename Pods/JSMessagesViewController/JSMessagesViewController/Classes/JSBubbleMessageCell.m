...
- (void)configureAvatarImageView:(UIImageView *)imageView forMessageType:(JSBubbleMessageType)type
{
    CGFloat avatarX = 4.0f;
    if(type == JSBubbleMessageTypeOutgoing) {
        avatarX = (self.contentView.frame.size.width - kJSAvatarImageSize) - 4.0f;
    }
    ...
    [imageLayer setCornerRadius:kJSAvatarImageSize/2];
				 ...
				 subtitle:(BOOL)hasSubtitle
{
    ...
    if(hasAvatar) {
        ...
        if(type == JSBubbleMessageTypeOutgoing) {
            offsetX = kJSAvatarImageSize - offsetX;
        }
                                          ...
                                          withFrame:(CGRect)frame
{
    ...
    CGFloat bubbleHeight = [JSBubbleView neededHeightForText:text withFrame:frame];
